...
The tables on the right are that dictionary. Three of them define it — the allowed values, the translations from each source, and the CMS codes. The other four hold each chart in dictionary terms. I have a detail page on them for when we sit down on it; I won't go through it here.
