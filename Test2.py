...
def test_get_cli_creds_success_non_qa(mock_load_config, mock_add_auth, mock_post, mock_iam_client_class):
    ...
    mock_iam_client.request_vault = Mock(return_value={"auth": "mock_auth_response"})
    ...
    mock_iam_client_class.assert_called_once_with(
        domain="https://example.com", role="vault_role", lockbox_id="lockbox_id"
    )
...
def test_get_cli_creds_success_non_qa(mock_add_auth, mock_post, mock_iam_client_class, mock_load_config):
    ...
    mock_iam_client.request_vault = Mock(return_value={"auth": {"client_token": "mock_token"}})
    ...
    result = get_cli_creds(chamber_config="chamber_config", env=env)
