...
# ──────────────────────────────────────────────────────────────────────
# ONE-LAKE branch  – uses get_partition() helper inside runEDQ.main()
# ──────────────────────────────────────────────────────────────────────
...
def test_main_onelake(self,
                      ...
                      mock_ol_cls,      # ②
                      mock_spark_cls):  # ①
    """Simple happy-path test for the current OneLake code path."""

    # 1️⃣  Force the onelake branch via config/secrets injection
    _inject_cfg(
        {
            "DATA_SOURCE": "onelake",
    ...
    dataset_mock.location = "s3://bucket/path/"          # base location
    ...
    def ls_side_effect(path):
        """Return a folder on 1st call, a file on 2nd."""
        call_count["n"] += 1
        if call_count["n"] == 1:
    ...
    self.assertEqual(s3fs_mock.ls.call_count, 2)                  # two listings
