...
def _make_status_stub(spark, cases):
    ...
    Keys are TYPED columns, not .str.
    """
    cases = list(cases)
    rows = [
    ...
    # Make sure join key name matches what UUT uses; alias only if names differ.
    return df.select(
...
def test_acc_update_delete_ind_rules(mock_calc_status, spark):
    ...
    # call UUT (match your real signature)
    ...
    expected_rows = [
        {ABSegment.account_id: a, ABSegment.ab_update_ind: expected_ind(s, d)}
        for a, s, d in cases
    ]
