...
def calculate_account_type(input_df: DataFrame) -> DataFrame:
    ...
    Expects a DataFrame containing at least:
      - credit_limit (string; may be blank, 'NPSL', or any other string)
      - product_type (string; e.g. 'small_business', 'private_label_partnership')
      - account_id   (unique identifier for the row)

    Returns a DataFrame with two columns:
      - account_id
      - account_type (string or None)
    
    Business Logic (in priority order):
      1) If trim(credit_limit) == ""  => account_type = None
      2) If product_type == "private_label_partnership" AND trim(credit_limit) == "NPSL"
         => account_type = None
      3) Else if trim(credit_limit) == "NPSL" => "0G"
      4) Else if product_type == "small_business" => "8A"
      5) Else if product_type == "private_label_partnership" => "07"
      6) Else => "18"
    """
