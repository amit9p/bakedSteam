...
Three: reproducibility. Every recommendation carries the versions that made it. CMS audits a chart two years from now, we re-run it and get the same answer. CMS audits, and a code we can't reproduce is a code we can't defend. Everything on the next slide is in service of those three.
...
Four lanes, four owners. CIS on top — not ours. Our data platform on Databricks. Reva's AI components — they exist as a POC, we host them. Rules and reference data at the bottom.
...
**Five.** The graph. Those facts stored as condition, evidence, doctor, all linked. Versioned. Where the audit trail lives.
